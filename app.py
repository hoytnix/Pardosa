...
class Pardosa:
    ...
    def fingerprint_technologies(self, html, soup):
        ...
        if 'clickfunnels' in html or 'cf2.0' in html:
            techs.append('ClickFunnels 2.0')
            
        # Kajabi
        if 'kajabi' in html or 'kajabi-assets' in html:
            techs.append('Kajabi')
            
        return techs

    def fingerprint_contact_info(self, html, soup, url):
        contact_info = {
            'phones': [],
            'emails': [],
            'social_media': [],
        ...
        for link in soup.find_all('a', href=True):
            ...
            for pattern, platform in social_patterns.items():
                if pattern in href:
                    contact_info['social_media'].append({'platform': platform, 'url': href})
        
        return contact_info

    async def save_fingerprints(self, fqdn, data):
        file_path = self.data_dir / f"{fqdn}.json"
